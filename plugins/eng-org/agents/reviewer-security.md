...
> **NOTE (v0.15.0-candidate, corrected in nit-fix-1):** This reviewer is
> retained **in the DEFAULT set** per the recorded RESHAPE deviation in
> `commands/run-reviews.md` §Step 2 (extractor gap on the overlap audit's
> 0-count security row + the §H always-rerun auth invariant). Default
> fan-out is 7 role reviewers + GR: `reviewer-governance` +
> `reviewer-domain-validator` + the 4 RESHAPE survivors named by
> `governance/requirements/REQ-20260713-d904-03/reviewer-overlap-audit.md`
> + this reviewer. A follow-up REQ, after ≥ 1 live campaign verifies the
> consolidation, may demote this reviewer to escalation-only or remove it.
...
The dev-report and test reports.
...
- REQ-20260713-d904-03 TASK-10 (Change 8a, enacted in fix-iteration-1): pruned the §Required reading list above to security-axis surfaces.
  - removed: `MISTAKES.md filter [auth, security, pii, ownership, secret, rate-limit]` — per-REQ curated MISTAKES slice arrives via the context pack.
